...
string DecodeSeriatedPlayfair(string Msg, string Key, bool DecodeAsDoubleLetter, int Period)
{
	string NewMsg;
	string Digram;
	NewMsg = "";
	...
	char Row1[Period];
	char Row2[Period];
	char OutputRow1[Period];
	char OutputRow2[Period];
	
	int i;
	int j;
	for (i = 0; i < Msg.length()+1-2*Period; i+= Period*2){
		for (j = 0; j < Period; j ++){
			//Row1[i] = Msg[i+j];
			Row1[j] = Msg[i+j];
		}
		...
		for (j = 0; j < Period; j++){
			Digram = Row1[j];
			Digram += Row2[j];
			//Digram = DecodePlayfair(Row1[j]+Row2[j], Key, DecodeAsDoubleLetter);
			Digram = DecodePlayfair(Digram, Key, DecodeAsDoubleLetter);
			OutputRow1[j] = Digram[0];
			OutputRow2[j] = Digram[1];
		}
		for (j = 0; j < Period; j++){
			NewMsg += OutputRow1[j];
		}
...
double Score(string Msg, string Key, bool DecodeAsDoubleLetter, int Period)
{
	string DecodedMsg;
	DecodedMsg = DecodeSeriatedPlayfair(Msg, Key, DecodeAsDoubleLetter, Period);
...
double PlayfairAnnealing(string Msg, string &OutputKey, bool DecodeAsDoubleLetter, int Period, float Temperature, float Step, int Count)
{
	double BestScore;
	...
	BestScore = Score(Msg, BestKey, DecodeAsDoubleLetter, Period);
	...
	for (T = Temperature; T >= 0; T -= Step){
		...
		for (Trial = 0; Trial < Count; Trial++){
			if (true){
				...
				CurrentScore = Score(Msg, CurrentKey, DecodeAsDoubleLetter, Period);
...
void SolvePlayfair(string Msg, bool DecodeAsDoubleLetter, int Period, string AlphabetForKey)
{
	...
	double BestScore = Score(Msg, BestKey, DecodeAsDoubleLetter, Period);
	...
	while (true){
		...
		//NewScore = PlayfairAnnealing(Msg, NewKey, DecodeAsDoubleLetter, Period, 20, 0.2, 10000);
		NewScore = PlayfairAnnealing(Msg, NewKey, DecodeAsDoubleLetter, Period, 1, 1, 10000);
		//NewScore = PlayfairAnnealing(Msg, NewKey, DecodeAsDoubleLetter, Period, 5, 1, 10000);
		...
		if (NewScore > BestScore){
			...
			DisplayKey(BestKey);
			cout << "\n";
			string Decipherment = DecodeSeriatedPlayfair(Msg, BestKey, DecodeAsDoubleLetter, Period);
...
int main(int argc, char *argv[])
{
	...
	ifstream File("Seriated Playfair Message.txt");
	...
	/*//Key = "hapybcdefgiklmnoqrstuvwxz";
	Key = "mrboziesagdfnchklpqtuvwxy";
	
	cout << DecodeSeriatedPlayfair(MESSAGE, Key, DECODEASDOUBLELETTER, PERIOD) << "\n";
	
	cin.get();*/
	
	cout << "Starting Solve!\n\n";
	
	SolvePlayfair(MESSAGE, DECODEASDOUBLELETTER, PERIOD, AlphabetForKey);
